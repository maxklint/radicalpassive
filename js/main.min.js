"use strict"
function main(){var galleries=buildGalleries()
registerKeyboardListener(galleries)
initNews()
if(document.body.hasAttribute("is-home"))
initHome()}
function registerKeyboardListener(galleries){document.addEventListener("keydown",function(event){galleries.forEach(function(gallery){if(!gallery.interactive)
return
...
function buildGalleries(){var galleries=[]
document.body.querySelectorAll(".gallery").forEach(function(gallery){galleries.push(new Gallery(gallery))})
return galleries}
...
this.has_thumbs=elem.hasAttribute("gallery-thumbs")
this.header=document.body.querySelector("header")
this.main=document.body.querySelector("main")
this.index=0
var images=elem.getAttribute("gallery-images").trim().split(",").filter(function(image){return image})
if(!images||images.length===0)
return
if(this.has_thumbs){if(images.length%3!=0)
return
this.size=images.length/3
this.fsimages=images.filter(function(image,i){return i%3==0})
this.thumbs=images.filter(function(image,i){return i%3==1})
this.thumbs2x=images.filter(function(image,i){return i%3==2})}else{this.size=images.length
this.fsimages=images}
...
setInterval(this.showNext.bind(this),5000)
...
if(this.has_thumbs&&!this.fullscreen)
img.setAttribute("srcset",this.thumbs[index]+" 1x, "+this.thumbs2x[index]+" 2x")
return img}
...
Gallery.prototype.attachControls=function(){if(this.size>1){this.attachControl(this.collapsed,"gallery__left-arrow","/icons/arrow_left.svg",this.showPrev)
